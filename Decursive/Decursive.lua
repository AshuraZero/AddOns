--[[
 Decursive (v 1.9.8.4) add-on for World of Warcraft UI
...
Dcr_Saved = {
    ...
    -- Fast range check helps to address the freeze problem some players encounted, at the cost of decreased max casting range of curing spells.
    FastRangeCheck = false;
    ...
    -- Cancel spells in progess
    AbortCurrentCast = false;
...
local Dcr_Unit_Array	    = { }; -- set to local  XXX

-- #############
-- CACHED TABLES (Re-used table to minimize garbege)
-- #############
...
-- The action bar slot which contains a curing spell icon for spell casting range check
local Dcr_Range_Slot = nil;
...
function MakePlayerName(Unit) --{{{
	local Name = (UnitName(Unit));

	local LocalizedClass, EnglishClass = UnitClass(Unit);

	if EnglishClass == DCR_CLASS_DRUID then
		return "|cFFFF7D0A|Hplayer:" .. Name .. "|h" .. Name .. "|h|r";
	elseif EnglishClass == DCR_CLASS_HUNTER then
		return "|cFFABD473|Hplayer:" .. Name .. "|h" .. Name .. "|h|r";
	elseif EnglishClass == DCR_CLASS_MAGE then
		return "|cFF69CCF0|Hplayer:" .. Name .. "|h" .. Name .. "|h|r";
	elseif EnglishClass == DCR_CLASS_PALADIN then
		return "|cFFF58CBA|Hplayer:" .. Name .. "|h" .. Name .. "|h|r";
	elseif EnglishClass == DCR_CLASS_PRIEST then
		return "|cFFFFFFFF|Hplayer:" .. Name .. "|h" .. Name .. "|h|r";
	elseif EnglishClass == DCR_CLASS_ROGUE then
		return "|cFFFFF569|Hplayer:" .. Name .. "|h" .. Name .. "|h|r";
	elseif EnglishClass == DCR_CLASS_SHAMAN then
		return "|cFFF58CBA|Hplayer:" .. Name .. "|h" .. Name .. "|h|r";
	elseif EnglishClass == DCR_CLASS_WARLOCK then
		return "|cFF9482C9|Hplayer:" .. Name .. "|h" .. Name .. "|h|r";
	elseif EnglishClass == DCR_CLASS_WARRIOR then
		return "|cFFC79C6E|Hplayer:" .. Name .. "|h" .. Name .. "|h|r";
	else
		return "|cFFFFAA22|Hplayer:" .. Name .. "|h" .. Name .. "|h|r";
	end
end --}}}

function MakeAfflictionName(name) --{{{
	if name == DCR_DISEASE then
		return "|cFF996600" .. DCR_LOC_AF_TYPE[name] .. "|r";
	elseif name == DCR_MAGIC then
		return "|cFF3399FF" .. DCR_LOC_AF_TYPE[name] .. "|r";
	elseif name == DCR_POISON then
		return "|cFF009900" .. DCR_LOC_AF_TYPE[name] .. "|r";
	elseif name == DCR_CURSE then
		return "|cFF9900FF" .. DCR_LOC_AF_TYPE[name] .. "|r";
	elseif name == DCR_CHARMED then
		return "|cFFCC0000" .. DCR_LOC_AF_TYPE[name] .. "|r";
	elseif name then
...
function Dcr_PrintPriorityList() --{{{
    for id, name in pairs(Dcr_Saved.PriorityList) do
	Dcr_println( id.." - "..name);
    end
end --}}}

function compoststats()
    compost:Stats();
end

function TMP_pr() --{{{
    Dcr_println("printing cure order:");
    for index, unit in ipairs(Dcr_Unit_Array) do
	Dcr_println(unit .. " - " .. MakePlayerName(unit) .. " Index: " .. index);
    end
end --}}}
...
function Dcr_SetCureOrderList () -- {{{
    ...
    local j = 0;
    local CheckBox;
    local temp_table = {}; -- we don't use compost here because this function is not called often and is critical.

    -- clear curing functions
    Curing_functions = compost:Erase(Curing_functions);
    for i=1, 4 do
...
function VerifyOrderList ()
    ...
    local TempTable = {}; -- we don't use compost here because this function is not called often and is critical.
    local i;
...
function Dcr_PriorityListEntryTemplate_OnClick(MouseButton, ShiftKeyDown) --{{{
	local id = this:GetID();

	if id then
		if ShiftKeyDown and ( (MouseButton == "LeftButton") or (MouseButton == "RightButton") ) then
			if (this.Priority) then
				Dcr_RemoveIDFromPriorityList(id);
			else
				Dcr_RemoveIDFromSkipList(id);
			end
		elseif (MouseButton == "LeftButton") and (id > 1) then
			if this.Priority then
				table.insert(Dcr_Saved.PriorityList, id - 1, Dcr_Saved.PriorityList[id]);
				Dcr_RemoveIDFromPriorityList(id + 1);
			else
				table.insert(Dcr_Saved.SkipList, id - 1, Dcr_Saved.SkipList[id]);
				Dcr_RemoveIDFromSkipList(id + 1);
			end
		elseif MouseButton == "RightButton" then
			if this.Priority then
				if id < table.getn(Dcr_Saved.PriorityList) then
					table.insert(Dcr_Saved.PriorityList, id + 2, Dcr_Saved.PriorityList[id]);
					Dcr_RemoveIDFromPriorityList(id);
				end
			else
				if id < table.getn(Dcr_Saved.SkipList) then
					table.insert(Dcr_Saved.SkipList, id + 2, Dcr_Saved.SkipList[id]);
					Dcr_RemoveIDFromSkipList(id);
				end
			end
		end
...
function Dcr_PriorityListFrame_OnUpdate() --{{{
	...
	if size < 1 then
		getglobal(baseName.."SliderLow"):SetText("");
		getglobal(baseName.."SliderHigh"):SetText("");
	else
		getglobal(baseName.."SliderLow"):SetText(1);
		getglobal(baseName.."SliderHigh"):SetText(size);
	end

	getglobal(baseName.."Slider"):SetMinMaxValues(0, size);
    end
...
function Dcr_AfflictedListFrame_OnUpdate(elapsed) --{{{
	...
	if (DCR_CAN_CURE_ENEMY_MAGIC) then
	    for _, unit in ipairs(Dcr_Unit_Array) do
...
function Dcr_ScanUnit( Unit, Index) --{{{
	...
	if (DCR_IGNORELIST[debuff_name]) then
	    ...
	    compost:Reclaim(AllUnitDebuffs, 1);
	    return false;
	end
...
function Dcr_UpdateLiveDisplay( Index, Unit, debuff_params) --{{{
    ...
    getglobal(baseFrame..Index.."Name"):SetText(MakePlayerNameByRange(Unit));

    getglobal(baseFrame..Index.."Class"):SetText(MakePlayerClass(Unit));
...
function MakePlayerNameByRange(Unit)
	local Name = (UnitName(Unit));
...
function MakePlayerClass(Unit)
	local LocalizedClass, EnglishClass = UnitClass(Unit);
	
	if EnglishClass == DCR_CLASS_DRUID then
		return "|cFFFF7D0A" .. LocalizedClass .. "|r";
	elseif EnglishClass == DCR_CLASS_HUNTER then
		return "|cFFABD473" .. LocalizedClass .. "|r";
	elseif EnglishClass == DCR_CLASS_MAGE then
		return "|cFF69CCF0" .. LocalizedClass .. "|r";
	elseif EnglishClass == DCR_CLASS_PALADIN then
		return "|cFFF58CBA" .. LocalizedClass .. "|r";
	elseif EnglishClass == DCR_CLASS_PRIEST then
		return "|cFFFFFFFF" .. LocalizedClass .. "|r";
	elseif EnglishClass == DCR_CLASS_ROGUE then
		return "|cFFFFF569" .. LocalizedClass .. "|r";
	elseif EnglishClass == DCR_CLASS_SHAMAN then
		return "|cFFF58CBA" .. LocalizedClass .. "|r";
	elseif EnglishClass == DCR_CLASS_WARLOCK then
		return "|cFF9482C9" .. LocalizedClass .. "|r";
	elseif EnglishClass == DCR_CLASS_WARRIOR then
		return "|cFFC79C6E" .. LocalizedClass .. "|r";
	else
		return "|cFFFFAA22" .. LocalizedClass .. "|r";
	end
end
...
function DcrAddUnitToPriorityList( unit) --{{{
	...
	if (UnitIsPlayer(unit)) then
	    local name = (UnitName( unit));
	    for _, pname in pairs(Dcr_Saved.PriorityList) do
		if (name == pname) then
		    return;
		end
	    end
...
function Dcr_ClearPriorityList() --{{{
    Dcr_Saved.PriorityList = compost:Erase(  Dcr_Saved.PriorityList    );
...
function DcrAddUnitToSkipList( unit) --{{{
	...
	if (UnitIsPlayer(unit)) then
	    local name = (UnitName( unit));
	    for _, pname in pairs(Dcr_Saved.SkipList) do
		if (name == pname) then
		    return;
		end
	    end
...
function Dcr_ClearSkipList() --{{{
    ...
    Dcr_Saved.SkipList = compost:Erase(  Dcr_Saved.SkipList	);
    --[[
...
function Dcr_PrintSkipList() --{{{
    for id, name in pairs(Dcr_Saved.SkipList) do
	Dcr_println( id.." - "..name);
    end
end --}}}

function Dcr_IsInPriorList (name) --{{{
    for _, PriorName in pairs(Dcr_Saved.PriorityList) do
...
function Dcr_IsInSkipList (name) --{{{
    for _, SkipName in pairs(Dcr_Saved.SkipList) do
...
function Dcr_Configure() --{{{
    ...
    DCR_SPELL_MAGIC_1 = compost:Recycle(DCR_SPELL_MAGIC_1, 0,"", "");
    DCR_SPELL_MAGIC_2 = compost:Recycle(DCR_SPELL_MAGIC_2, 0,"", "");
    DCR_CAN_CURE_MAGIC = false;
    DCR_SPELL_ENEMY_MAGIC_1 = compost:Recycle(DCR_SPELL_ENEMY_MAGIC_1, 0,"", "");
    DCR_SPELL_ENEMY_MAGIC_2 = compost:Recycle(DCR_SPELL_ENEMY_MAGIC_2, 0,"", "");
    DCR_CAN_CURE_ENEMY_MAGIC = false;
    DCR_SPELL_DISEASE_1 = compost:Recycle(DCR_SPELL_DISEASE_1, 0,"", "");
    DCR_SPELL_DISEASE_2 = compost:Recycle(DCR_SPELL_DISEASE_2, 0,"", "");
    DCR_CAN_CURE_DISEASE = false;
    DCR_SPELL_POISON_1 = compost:Recycle(DCR_SPELL_POISON_1, 0,"", "");
    DCR_SPELL_POISON_2 = compost:Recycle(DCR_SPELL_POISON_2, 0,"", "");
    DCR_CAN_CURE_POISON = false;
    DCR_SPELL_CURSE = compost:Recycle(DCR_SPELL_CURSE, 0,"", "");
    DCR_CAN_CURE_CURSE = false;
	...
	while (true) do -- I wish there was a continue statement in LUA...
		...
		icon = GetSpellTexture(i, BookType)
    ...
    Dcr_Range_Slot = Dcr_FindCureingActionSlot(Dcr_CuringAction_Icons);

    if ( DCR_HAS_SPELLS and (not Dcr_Range_Slot) and (Dcr_PlayerClass ~= DCR_CLASS_WARLOCK) ) then
...
function Dcr_OnEvent (event) --{{{
    ...
    elseif (event == "CHAT_MSG_RAID") or (event == "CHAT_MSG_RAID_LEADER") then
    	Dcr_UserCheckResponse(arg1, arg2);
    	return;
    end
...
function Dcr_OnUpdate(arg1) --{{{
    ...
    for unit in pairs(Dcr_Blacklist_Array) do
	...
	if (Dcr_Debuff_Texture_to_name_cache_life < 0) then
	    ...
	    Dcr_Debuff_Texture_to_name_cache = compost:Erase(Dcr_Debuff_Texture_to_name_cache);
	...
	if (Dcr_Buff_Texture_to_name_cache_life < 0) then
	    ...
	    Dcr_Buff_Texture_to_name_cache = compost:Erase(Dcr_Buff_Texture_to_name_cache);
...
function Dcr_GetUnitArray() --{{{
    ...
    InternalPrioList	    = compost:Erase (  InternalPrioList		);
    InternalSkipList	    = compost:Erase (  InternalSkipList		);
    SortingTable	    = compost:Erase (  SortingTable		);
    Dcr_Unit_ArrayByName    = compost:Erase (  Dcr_Unit_ArrayByName	);

    local unit;
    -- First sort the prioritylist (remove missing units) and add its content to the main list
    for _, pname in pairs(Dcr_Saved.PriorityList) do
	unit = Dcr_NameToUnit( pname );
	if (unit) then
	    InternalPrioList[pname] = unit;
	    -- use this loop to add prio characters to the main list
	    Dcr_Unit_ArrayByName[pname] = unit;
	...
	Dcr_Unit_ArrayByName[MyName] = "player";
	...
	if (UnitExists("party"..i)) then
		...
		-- we don't skip him
		Dcr_Unit_ArrayByName[pname] = "party"..i;
	...
	for i = 1, raidnum do
		...
		if (not RaidRosterCache[i]) then
		    RaidRosterCache[i] = compost:Acquire();
		end

		if (not rName) then -- at logon sometimes rName is nil...
		    rName = rGroup.."unknown"..i;
		end
		RaidRosterCache[i].rName    = rName;
		RaidRosterCache[i].rGroup   = rGroup;
		RaidRosterCache[i].rIndex   = i; -- I can't trust lua, the manual is not clear at all about table behavior...
	    elseif ( RaidRosterCache[i] ) then
		RaidRosterCache[i] = compost:Erase(RaidRosterCache[i]);
	    end
	...
	for _, raidMember in ipairs(RaidRosterCache) do
		...
		if (raidMember.rGroup > currentGroup) then
		    Dcr_Unit_ArrayByName[raidMember.rName] = "raid" .. raidMember.rIndex;
	...
	if (UnitExists("pet")) then
	    Dcr_Unit_ArrayByName[(UnitName("pet"))] = "pet";
	...
	for i = 1, 4 do
		...
		Dcr_Unit_ArrayByName[pname] = "partypet"..i;
	...
	if (raidnum > 0) then
		...
		if (UnitExists("raidpet"..i)) then
		    ...
		    if (not Dcr_Unit_ArrayByName[pname]) then
			Dcr_Unit_ArrayByName[pname] = "raidpet"..i;
    ...
    -- we use a hash-key style table for Dcr_Unit_ArrayByName because it allows us
    -- to not care if we add a same unit several times (speed optimization)
    -- but we cannot use sort unless indexes are integer so:
    Dcr_Unit_Array = compost:Erase(Dcr_Unit_Array);
    for _, value in pairs(Dcr_Unit_ArrayByName) do -- /!\ PAIRS not iPAIRS
	table.insert(Dcr_Unit_Array, value);
    end
...
function Dcr_Clean(UseThisTarget, SwitchToTarget) --{{{
    ...
    DCR_ThisCleanBlaclisted = compost:Erase(  DCR_ThisCleanBlaclisted );
	...
	if (targetEnemy) then
	    -- we had something "bad" targeted, if the current target is something good then switch back to the bad
...
function Dcr_GetUnitDebuff  (Unit, i) --{{{
    local DebuffTexture, debuffApplications, debuff_type = UnitDebuff(Unit, i);
...
function Dcr_GetUnitDebuffName  (Unit, i, DebuffTexture) --{{{
	...
	-- Dcr_debug_bis("Debuff cache USED !"); -- XXX
    end
...
function Dcr_GetUnitDebuffAll (unit) --{{{
    ...
    local ThisUnitDebuffs = compost:Acquire();
	...
	ThisUnitDebuffs[debuff_name] = compost:Acquire();
...
function Dcr_CheckUnitForBuffs(Unit, BuffNamesToCheck) --{{{

    --[[
    XXX in WOW 2.0, two new API functions exists:
	    UnitHasBuff("unit", "name"[, "rank"]) 
	    UnitHasDebuff("unit", "name"[, "rank"])

    --]]
    local CheckSeveral = true;
    if type(BuffNamesToCheck) ~= "table" then
	CheckSeveral = false;
    end

    UnitBuffs = compost:Erase(UnitBuffs);
	...
	if (not Dcr_Buff_Texture_to_name_cache[texture]) then
	    ...
	    -- Dcr_debug_bis("Buff cache USED !");
	    found_buff_name = Dcr_Buff_Texture_to_name_cache[texture];
	end

	table.insert(UnitBuffs, found_buff_name);

	if (not CheckSeveral) then
	    return (found_buff_name == BuffNamesToCheck);
	end
    ...
    local Buff;
    FoundBuffs = compost:Erase(FoundBuffs);
    -- check for debuffs we have to find
    for _, Buff in pairs(UnitBuffs) do

	if BuffNamesToCheck[Buff] then
...
function Dcr_CheckUnitStealth(Unit) --{{{
	...
	if Dcr_CheckUnitForBuffs(Unit, DCR_INVISIBLE_LIST) then
	    Dcr_debug_bis("Sealth found !");
	    return true;
	end
    end
    return false;
...
function Dcr_UnitInRange(Unit) -- {{{
	...
	if Dcr_Saved.FastRangeCheck then
		return false;
	end

	if (not Dcr_Range_Slot) then
		return false;
	end
...
function Dcr_FindCureingActionSlot(iconArray) -- {{{
		...
		if (HasAction(i)) then
			...
			if (iconArray[icon]) then
				if (GetActionText(i) == nil) then
					spellName = iconArray[icon];
					Dcr_ScanningTooltipTextLeft1:SetText("");
					Dcr_ScanningTooltip:SetAction(i);
					slotName = Dcr_ScanningTooltipTextLeft1:GetText();
					if (spellName == slotName) then
						Dcr_debug_bis("Decursive found curing spell '" .. spellName .. "' in your action bar slot " .. tostring(i) .. ".");
						return i;
					end
				else
	...
	Dcr_debug_bis("Decursive did not find any curing spell in your action bars.");
	return nil;
end -- // }}}
-- }}}
...
function Dcr_CureUnit(Unit)  --{{{
    ...
    -- test here for the cooldown and cancel current casting operations if there is something to cure
    

    if (
	(Magic_Count > 0 or Curse_Count > 0 or Poison_Count > 0 or Disease_Count > 0)
	and Dcr_Saved.AbortCurrentCast and Dcr_PlayerClass ~= DCR_CLASS_WARLOCK
	) then
	...
	Dcr_debug_bis("--> Cooldown is: "..cooldown);
	return true; -- act like if something was done to prevent a useless loop over all cursed people
    end

    local res = false;
    local counts = compost:Acquire();
...
function Dcr_Cure_Poison(counts, Unit)  -- {{{
    ...
    if (Dcr_Saved.Check_For_Abolish and Dcr_CheckUnitForBuffs(Unit, DCR_SPELL_ABOLISH_POISON)) then
	return false;
    end
...
function Dcr_Cure_Disease(counts, Unit) --{{{
    ...
    if (Dcr_Saved.Check_For_Abolish and Dcr_CheckUnitForBuffs(Unit, DCR_SPELL_ABOLISH_DISEASE)) then
	return false;
    end
...
function Dcr_Cast_CureSpell( spellID, Unit, AfflictionType, ClearCurrentTarget) --{{{
	...
	Dcr_errln( string.gsub( string.gsub(DCR_OUT_OF_RANGE, "$t", MakePlayerName(Unit)), "$a", MakeAfflictionName(AfflictionType)));
    ...
    Dcr_println( string.gsub( string.gsub(DCR_CLEAN_STRING, "$t", MakePlayerName(Unit)), "$a", MakeAfflictionName(AfflictionType)));
...
function Dcr_UserCheckResponse(arg1, arg2)
	if Dcr_CombatMode then
		return;
	end

	local Msg = string.upper(arg1);
	local Sender = arg2;

	if string.find(Msg, DCR_USER_CHECK_DETECT1) and string.find(Msg, DCR_USER_CHECK_DETECT2) then
		local RaidNum = GetNumRaidMembers();
		local i;
		local Name, Rank;
		for i = 1, RaidNum do
			...
			if (Name == Sender) then
				if (Rank > 0) then
					SendChatMessage(format(DCR_USER_CHECK_RESPONSE, UnitName("player"), UnitClass("player"), DCR_VERSION_STRING), "RAID");
				end
				break;
			end
		end
