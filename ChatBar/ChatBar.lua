...
if (Sky) then
	...
	--		
	--
	ChatBar_ReorderChannels = function()
	...
	ChatBar_joinChannelsInOrder = function(newChannelOrder)
	...
	ChatBar_leaveExtraChannels = function(newChannelOrder)
		
		for i, bogusName in BOGUS_CHANNELS do
			local channelNum, channelName = GetChannelName(bogusName);
			if (channelName) then
				LeaveChannelByName(channelNum, true);
			end
		end
