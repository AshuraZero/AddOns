...
function module:OnEngage()
	...
	self:TriggerEvent("BigWigs_StartCounterBar", self, "摧毁蛋", 30, "Interface\\Icons\\inv_egg_01")
	self:TriggerEvent("BigWigs_SetCounterBar", self, "摧毁蛋", (30 - 0.1))
end
...
function module:BigWigs_RecvSync(sync, rest, nick)
    if sync == syncName.egg then
		...
		if rest == (self.eggs + 1) and self.eggs <= 30 then
            ...
            self:TriggerEvent("BigWigs_SetCounterBar", self, "摧毁蛋", (30 - self.eggs))
    ...
    elseif sync == syncName.phase2 and self.phase < 2 then
		...
		self:TriggerEvent("BigWigs_StopCounterBar", self, "摧毁蛋")
