...
L:RegisterTranslations("zhCN", function() return {
	...
	vulnerability_direct_test = "^[%w]+[%s的]*([%w%s:]+)([%w]+)克洛玛古斯造成([%d]+)点([%w]+)伤害%。[%s%(]*([%d]*点被抵抗)", -- [Fashu's] [Firebolt] [hits] Battleguard Sartura for [44] [Fire] damage. ([14] resisted)
	vulnerability_dots_test = "^克洛玛古斯的([%d]+)使([%w]+)受到了([^%s]+)%点([%w]+)伤害。",
	frenzy_trigger = "goes into a killing frenzy",
	frenzyfade_trigger = "效果从克洛玛古斯身上消失。",
