...
L:RegisterTranslations("zhCN", function() return {
	...
	["Always show in raid"] = "总是显示在RAID",
	["Always show in raid after ignite tick"] = "点燃总是显示在RAID",
	...
	fire_test = "^([%w]+)的(.+)致命一击对(.+)造成([%d]+)点([%w]+)伤害。",
	ignite_stack_test = "^(.+)受到了点燃效果的影响。[%s%(]*([%d]*)",
	ignite_damage_test = "^([%w]+)的点燃使(.+)受到了([%d]+)点火焰伤害。",
	ignite_resist = "([%d]+)点被抵抗",
	ignite_fade_test = "^点燃效果从(.+)身上消失。",
