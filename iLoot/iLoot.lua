...
function iLoot_SlashCommandHandler(cmd)
    ...
    if(string.lower(cmds[1]) == "enable") then
        iLoot_Enabled = true
        iLoot_showstat()
    elseif(string.lower(cmds[1]) == "disable") then
        iLoot_Enabled = false
        iLoot_showstat()
    elseif(string.lower(cmds[1]) == "stat") then
        iLoot_showstat()
    elseif(string.lower(cmds[1]) == "") then
        iLoot_Message("iLoot 设置命令\n/iloot enable |CFFFFFF00--启用|R\n/iloot disable --|CFFFFFF00禁用")
    else
        iLoot_Message("iLoot 设置命令\n/iloot enable |CFFFFFF00--启用|R\n/iloot disable --|CFFFFFF00禁用")
    end
end
                ...
                if ((name == player) and targetname and targetdead and boss) then
                    if ((GetTime() - iloottime > 60) or (ilootcount < 3)) then
                        if(GetTime() - iloottime > 60) then
                            SendChatMessage("黑手通知：> >"..name.."< <".." 打开了 "..targetname.." 的尸体","RAID")    
                            iLoot_Message("|CFFF00000".."> >"..name.."< <".."|R".." 打开了 ".."|CFFFFFF00"..targetname.."|R".." 的尸体")
                            iloottime = GetTime()
                            ilootcount = 0
                        end
