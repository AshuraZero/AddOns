...
function MobResist_Command(arg)
	...
	if arg1 == "reset" then
		MobResistDisplay:SetPoint("TOPLEFT", 200, -200);
	end
...
function TargetCheck(unit)
	...
	if not n or UnitIsCorpse(unit) or UnitIsDead(unit) or UnitPlayerControlled(unit) then 
		MR_Target = nil;
		return false;
	else
		MR_Target = unit
...
function Update()
	...
	if (TargetCheck("target")) then
		...
		ArmorText:SetText(GetArmor().." 护甲");
		HolyText:SetText(GetHoly().." 神圣");
		FireText:SetText(GetFire().." 火抗");
		NatureText:SetText(GetNature().." 自然抗");
		FrostText:SetText(GetFrost().." 冰抗");
		ShadowText:SetText(GetShadow().." 暗抗");
		ArcaneText:SetText(GetArcane().." 奥抗");
	else
		MobResistDisplay:Hide();
		NameText:SetText("")
		ArmorText:SetText("");
...
function Report(channel)
	...
	local message = GetTargetName().." has "..GetArmor().." Armor, "..GetHoly().." Holy, "..GetFire().." Fire, "..GetNature().." Nature, "..GetFrost().." Frost, "..GetShadow().." Shadow and "..GetArcane().." Arcane Resistance.";
