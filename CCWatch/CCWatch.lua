...
-- register this also for < 60 (pvp)
	...
	--CCWatch_AddMessage(CCWATCH_VERSION..CCWATCH_LOADED);
end
...
function CCWatch_EffectGone(effect)
	local i;
	local effectgone = true; -- assume effect is gone unless we find it
	...
	if effectgone then
		for i = 1,MAX_TARGET_BUFFS do
			if UnitBuff("target", i) == CCWATCH.CCS[effect].TEXTURE then
				effectgone = false
			end
		end
	end
			...
			EarthFeature_AddButton(
				{
					id="CCWATCH";
					name="CCWatch";
					subtext=CCWATCH_OPTIONS_TITLE;
					helptext=CCWATCH_DESCRIPTION;
					icon="Interface\\Icons\\Ability_Rogue_KidneyShot";
					callback=CCWatchOptions_Toggle;
				}
			);
		end
		CCWatch_LoadVariables();
