...
function CCWatchOptions_RogueCCToggle()
	...
	CCWATCH.CCS[CCWATCH_DISTRACT].MONITOR = bState;
end
...
function CCWatchOptions_MonitorCCToggle()
	CCWATCH.MONITORCC = not CCWATCH.MONITORCC;
	if CCWATCH.MONITORDEBUFF == false then
		if CCWATCH.MONITORCC == true then
...
function CCWatchOptions_MonitorDebuffToggle()
	CCWATCH.MONITORDEBUFF = not CCWATCH.MONITORDEBUFF;
	if CCWATCH.MONITORCC == false then
		if CCWATCH.MONITORDEBUFF == true then
			CCWatchObject:RegisterEvent("CHAT_MSG_SPELL_PERIODIC_CREATURE_DAMAGE");
			CCWatchObject:RegisterEvent("CHAT_MSG_SPELL_PERIODIC_HOSTILEPLAYER_DAMAGE");
		else
			CCWatchObject:UnregisterEvent("CHAT_MSG_SPELL_PERIODIC_CREATURE_DAMAGE");
...
function CCWatchOptions_MonitorBuffToggle()
	CCWATCH.MONITORBUFF = not CCWATCH.MONITORBUFF;
	if CCWATCH.MONITORBUFF == true then
...
function CCWatchOptionsLearnFillFields(mod)
	...
	if CCWATCH.CCS[CCWatchEffectSelection].ETYPE == 2 then
		CCWatchOptionsEffectTypeDropDownText:SetText("BUFF");
	elseif CCWATCH.CCS[CCWatchEffectSelection].ETYPE == 1 then
		CCWatchOptionsEffectTypeDropDownText:SetText("DEBUFF");
	else
		CCWatchOptionsEffectTypeDropDownText:SetText("CC");
	end
...
function CCWatchOptions_Init()
	...
	CCWatchOptionsFrameMonitorCC:SetChecked(CCWATCH.MONITORCC);
	CCWatchOptionsFrameMonitorDebuff:SetChecked(CCWATCH.MONITORDEBUFF);
	CCWatchOptionsFrameMonitorBuff:SetChecked(CCWATCH.MONITORBUFF);
...
--	CCWatch_AddMessage("CCWatchOptionsEffects_Update");
	...
	CCcount = 0;
	table.foreach(CCWATCH.CCS, function(k,v) CCcount = CCcount + 1; end);
...
--	CCWatch_AddMessage("We're at "..curoffset);
	...
	table.foreach(CCWATCH.CCS, EffectsUpdate);
end
...
--	CCWatch_AddMessage("CCWatchOptionsEffects_OnEnter");
	...
	if CCWATCH.CCS[spellname].ETYPE == 2 then
		str = str.."Buff";
	elseif CCWATCH.CCS[spellname].ETYPE == 1 then
		str = str.."DeBuff";
	else
		str = str.."CC";
	end
