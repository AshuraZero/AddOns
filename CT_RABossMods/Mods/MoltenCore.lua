...
function CT_RABoss_Majordomo_OnLoad()
	...
	CT_RABoss_AddDropDownButton("Majordomo", { CT_RABOSS_DOMO_HIDE_5SEC, CT_RABOSS_DOMO_HIDE_5SEC_INFO }, "CT_RABoss_ModInfo", "priorWarning", "CT_RABoss_SetInfo");
end
...
function CT_RABoss_Majordomo_EventHandler(event)
	...
	if ( event == "CHAT_MSG_SPELL_PERIODIC_CREATURE_BUFFS" ) then
		if ( string.find(arg1, CT_RABOSS_DOMO_REFLECT_GAIN) and not CT_RABoss_Mods["Majordomo"]["AurasUp"] ) then
			...
			CT_RABoss_Announce(CT_RABOSS_DOMO_REFLECTWARN, CT_RABoss_Mods["Majordomo"]["announce"]);
...
function CT_RABoss_BaronGeddon_OnLoad()
	...
	CT_RABoss_AddEvent("Baron Geddon", "CHAT_MSG_SPELL_PERIODIC_CREATURE_BUFFS", CT_RABoss_BaronGeddon_EventHandler);
	
	CT_RABoss_AddDropDownButton("Baron Geddon", { CT_RABOSS_ANNOUNCE, CT_RABOSS_ANNOUNCE_INFO .. CT_RABOSS_REQ_LEADER_OR_PROM }, "CT_RABoss_ModInfo", "announce", "CT_RABoss_SetInfo");	
	...
	CT_RABoss_AddDropDownButton("Baron Geddon", { CT_RABOSS_BARON_INFERNO, CT_RABOSS_BARON_INFERNO_INFO }, "CT_RABoss_ModInfo", "inferno", "CT_RABoss_SetInfo");
...
function CT_RABoss_BaronGeddon_EventHandler(event)
	...
	if ( event == "preInferno" and CT_RABoss_Mods["Baron Geddon"]["inferno"] ) then
		CT_RABoss_Mods["Baron Geddon"]["infernoTrip"] = false;
		CT_RABoss_Announce(CT_RABOSS_BARON_INFERNO_PREWARN, CT_RABoss_Mods["Baron Geddon"]["announce"]);
		CT_RABoss_PlaySound(3);

	elseif ( event == "CHAT_MSG_SPELL_PERIODIC_CREATURE_BUFFS" and arg1 == CT_RABOSS_BARON_INFERNO_DETECT and CT_RABoss_Mods["Baron Geddon"]["inferno"] and not CT_RABoss_Mods["Baron Geddon"]["infernoTrip"] ) then
		CT_RABoss_Mods["Baron Geddon"]["infernoTrip"] = true;
		CT_RABoss_Announce(CT_RABOSS_BARON_INFERNO_WARN, CT_RABoss_Mods["Baron Geddon"]["announce"]);
		CT_RABoss_PlaySound(2);
		CT_RABoss_Schedule("CT_RABoss_BaronGeddon_EventHandler", 19, "preInferno");
		CT_RABoss_VisualTimer.addTimer("GeddonInferno", 21, CT_RABOSS_BARON_INFERNO_TIMER);
	...
	if ( event == "CHAT_MSG_SPELL_PERIODIC_SELF_DAMAGE" or event == "CHAT_MSG_SPELL_PERIODIC_FRIENDLYPLAYER_DAMAGE" or event == "CHAT_MSG_SPELL_PERIODIC_PARTY_DAMAGE" ) then
		local _, _, sPlayer = string.find(arg1, CT_RABOSS_BARON_AFFLICT_BOMB);
		
		if ( sPlayer ) then
			if ( sPlayer == CT_RABOSS_SELF1 ) then
				CT_RABoss_Announce(CT_RABOSS_BARON_BOMBWARNYOU);
				CT_RABoss_Announce(CT_RABOSS_BARON_BOMBWARNYOU);
				if ( CT_RA_Level >= 1 and CT_RABoss_Mods["Baron Geddon"]["announce"] ) then
					SendChatMessage("*** " .. UnitName("player") .. CT_RABOSS_BARON_BOMBWARNRAID, "RAID_WARNING");
				end
				CT_RABoss_PlaySound(2);
				
			else
				if ( CT_RABoss_Mods["Baron Geddon"]["sendTell"] and CT_RA_Level >= 1 ) then
					SendChatMessage(CT_RABOSS_BARON_BOMBWARNTELL, "WHISPER", nil, sPlayer);
				end
				if ( CT_RABoss_Mods["Baron Geddon"]["alertNearby"] ) then
					CT_RABoss_Announce("*** " .. sPlayer .. CT_RABOSS_BARON_BOMBWARNRAID, CT_RABoss_Mods["Baron Geddon"]["announce"]);
					CT_RABoss_PlaySound(2);
				end
			end
		end
...
function CT_RABoss_Magmadar_OnLoad()
	...
	CT_RABoss_AddDropDownButton("Magmadar", { CT_RABOSS_MAGMADAR_FA, CT_RABOSS_MAGMADAR_FA_INFO }, "CT_RABoss_ModInfo", "frenzyAlert", "CT_RABoss_SetInfo");
	CT_RABoss_AddDropDownButton("Magmadar", { CT_RABOSS_MAGMADAR_FEAR, CT_RABOSS_MAGMADAR_FEAR_INFO }, "CT_RABoss_ModInfo", "AEFear", "CT_RABoss_SetInfo");
end
...
function CT_RABoss_Magmadar_EventHandler(event)
	...
	elseif ( (string.find(arg1, CT_RABOSS_MAGMADAR_PANIC)) and not CT_RABoss_Mods["Magmadar"]["FearTrip"] and CT_RABoss_Mods["Magmadar"]["AEFear"] ) then
		CT_RABoss_Mods["Magmadar"]["FearTrip"] = true;
		CT_RABoss_Announce(CT_RABOSS_MAGMADAR_33SECWARN, CT_RABoss_Mods["Magmadar"]["announce"]);
		CT_RABoss_PlaySound(2);
...
function CT_RABoss_Gehennas_EventHandler(event)
	...
	elseif ( (string.find(arg1, CT_RABOSS_GEHENNAS_CURSE)) and not CT_RABoss_Mods["Gehennas"]["CurseTrip"] ) then
		...
		CT_RABoss_VisualTimer.addTimer("GehennasCurse", 29, CT_RABOSS_GEHENNAS_CURSE_TIMER);
	end
end
...
function CT_RABoss_Ragnaros_OnLoad()
	...
	CT_RABoss_AddEvent("Ragnaros", "CHAT_MSG_COMBAT_CREATURE_VS_PARTY_HITS", CT_RABoss_Ragnaros_EventHandler);
	CT_RABoss_AddEvent("Ragnaros", "CHAT_MSG_COMBAT_CREATURE_VS_PARTY_MISSES", CT_RABoss_Ragnaros_EventHandler);
	CT_RABoss_AddEvent("Ragnaros", "CHAT_MSG_COMBAT_CREATURE_VS_SELF_HITS", CT_RABoss_Ragnaros_EventHandler);
	CT_RABoss_AddEvent("Ragnaros", "CHAT_MSG_COMBAT_CREATURE_VS_SELF_MISSES", CT_RABoss_Ragnaros_EventHandler);
	CT_RABoss_AddEvent("Ragnaros", "CHAT_MSG_SPELL_CREATURE_VS_PARTY_DAMAGE", CT_RABoss_Ragnaros_EventHandler);
	CT_RABoss_AddEvent("Ragnaros", "CHAT_MSG_SPELL_CREATURE_VS_SELF_DAMAGE", CT_RABoss_Ragnaros_EventHandler);
	CT_RABoss_AddEvent("Ragnaros", "CHAT_MSG_COMBAT_PARTY_HITS", CT_RABoss_Ragnaros_EventHandler);
	CT_RABoss_AddEvent("Ragnaros", "CHAT_MSG_COMBAT_PARTY_MISSES", CT_RABoss_Ragnaros_EventHandler);
	CT_RABoss_AddEvent("Ragnaros", "CHAT_MSG_COMBAT_SELF_HITS", CT_RABoss_Ragnaros_EventHandler);
	CT_RABoss_AddEvent("Ragnaros", "CHAT_MSG_COMBAT_SELF_MISSES", CT_RABoss_Ragnaros_EventHandler);
	...
	CT_RABoss_SetVar("Ragnaros", "WaitStart", 0);
	...
	CT_RA_RegisterSlashCmd("/ragstart", CT_RABOSS_CMD_RAGSTART, 30, "RAGSTART", function()
		...
		CT_RABoss_SetVar("Ragnaros", "WaitStart", 1);
	end, "/ragstart");
end
...
function CT_RABoss_Ragnaros_EventHandler(event)
	...
	if ( event == "RagUp" or ( (event == "CHAT_MSG_MONSTER_YELL") and (string.find(arg1, CT_RABOSS_RAGNAROS_START)) ) ) then
		...
		CT_RABoss_VisualTimer.addTimer("RagnarosKnockback", 27, CT_RABOSS_RAGNAROS_KNOCKBACK_TIMER);
	
	elseif ( CT_RABoss_Mods["Ragnaros"]["WaitStart"] == 1 and string.find(arg1, CT_RABOSS_NAMES["Ragnaros"]) and UnitName("target") == CT_RABOSS_NAMES["Ragnaros"] and ( (event == "CHAT_MSG_COMBAT_CREATURE_VS_PARTY_HITS") or (event == "CHAT_MSG_COMBAT_CREATURE_VS_PARTY_MISSES") or (event == "CHAT_MSG_COMBAT_CREATURE_VS_SELF_HITS") or (event == "CHAT_MSG_COMBAT_CREATURE_VS_SELF_MISSES") or (event == "CHAT_MSG_SPELL_CREATURE_VS_PARTY_DAMAGE") or (event == "CHAT_MSG_SPELL_CREATURE_VS_SELF_DAMAGE") or (event == "CHAT_MSG_COMBAT_PARTY_HITS") or (event == "CHAT_MSG_COMBAT_PARTY_MISSES") or (event == "CHAT_MSG_COMBAT_SELF_HITS") or (event == "CHAT_MSG_COMBAT_SELF_MISSES") ) ) then
		CT_RABoss_SetVar("Ragnaros", "WaitStart", 0);
		CT_RABoss_UnSchedule("CT_RABoss_Ragnaros_EventHandler"); -- Remove outstanding RagUpWarn if all sons die before 15 second warning
		...
		CT_RABoss_VisualTimer.addTimer("RagnarosSons", 180, CT_RABOSS_RAGNAROS_SONS_TIMER);
		CT_RABoss_Schedule("CT_RABoss_Ragnaros_EventHandler", 23, "preRagKB");
		CT_RABoss_VisualTimer.addTimer("RagnarosKnockback", 10, CT_RABOSS_RAGNAROS_KNOCKBACK_TIMER);
...
function CT_RABoss_Shazzrah_OnLoad()
	...
	CT_RABoss_AddEvent("Shazzrah", "CHAT_MSG_SPELL_CREATURE_VS_CREATURE_DAMAGE", CT_RABoss_Shazzrah_EventHandler);
...
function CT_RABoss_Shazzrah_EventHandler(event)
	...
	if ( (event == "CHAT_MSG_SPELL_PERIODIC_CREATURE_BUFFS" or event == "CHAT_MSG_SPELL_CREATURE_VS_CREATURE_DAMAGE") and string.find(arg1, CT_RABOSS_SHAZZRAH_BLINK) ) then
		CT_RABoss_Announce(CT_RABOSS_SHAZZRAH_40SECSBLINK, CT_RABoss_Mods["Shazzrah"]["announce"]);
		CT_RABoss_PlaySound(2);
...
function CT_RABoss_Lucifron_EventHandler(event)
	...
	elseif ( ( string.find(arg1, CT_RABOSS_LUCIFRON_CURSE) ) and not CT_RABoss_Mods["Lucifron"]["CurseTrip"] ) then
		...
		CT_RABoss_VisualTimer.addTimer("LucifronCurse", 15, CT_RABOSS_LUCIFRON_CURSE_TIMER);
	...
	elseif ( ( string.find(arg1, CT_RABOSS_LUCIFRON_DOOM) ) and not CT_RABoss_Mods["Lucifron"]["ImpendingTrip"] and CT_RABoss_Mods["Lucifron"]["enableDoom"] ) then
		...
		CT_RABoss_VisualTimer.addTimer("LucifronDoom", 15, CT_RABOSS_LUCIFRON_DOOM_TIMER);
	end
end
