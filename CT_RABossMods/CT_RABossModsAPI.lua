...
CT_RABoss_Locations = {
	{ CT_RABOSS_LOCATIONS_NAXXRAMAS, 0 }, { CT_RABOSS_LOCATIONS_AHNQIRAJ, 0 }, { CT_RABOSS_LOCATIONS_RUINSOFAHNQIRAJ, 0 }, { CT_RABOSS_LOCATIONS_MOLTENCORE, 0 }, { CT_RABOSS_LOCATIONS_BLACKWINGSLAIR, 0 }, { CT_RABOSS_LOCATIONS_ONYXIASLAIR, 0 }, { CT_RABOSS_LOCATIONS_ZULGURUB, 0 }, { CT_RABOSS_LOCATIONS_OUTDOOR, 0 }, { CT_RABOSS_LOCATIONS_OTHER, 0 }
};
...
function CT_RABoss_Schedule(nameOrFunction, timeUntil, event, ... )
	if (type(timeUntil) == "number" and timeUntil > 0) then
		tinsert(CT_RABoss_ScheduledActions, { nameOrFunction, GetTime()+timeUntil, event, arg });
	end
end

-- Function to unschedule all functions where the first index is "name"
function CT_RABoss_UnSchedule(name, event)
	local v;
	for k = getn(CT_RABoss_ScheduledActions), 1, -1 do
		v = CT_RABoss_ScheduledActions[k];
		if ( v[1] == name and ( not event or v[3] == event ) ) then
...
function CT_RABoss_ScanModZones()
	...
	for k, v in CT_RABoss_Mods do
		if ( v["location"] ) then
			...
			elseif ( v["location"] == CT_RABOSS_LOCATIONS_AHNQIRAJ ) then
				CT_RABoss_Mods[k].enabled = ( zone == CT_RABOSS_MINIMAPLOC_AHNQIRAJ );
			elseif ( v["location"] == CT_RABOSS_LOCATIONS_RUINSOFAHNQIRAJ ) then
				CT_RABoss_Mods[k].enabled = ( zone == CT_RABOSS_MINIMAPLOC_RUINSOFAHNQIRAJ );
...
function CT_RABoss_AddMod(modName, modDescript, modStatus, modLocation)
	if ( not modLocation ) then
		modLocation = CT_RABOSS_LOCATIONS_OTHER;
	end
...
-- Announce function
function CT_RABoss_Announce(msg, fullRaid, hideLocal)
	if ( fullRaid and CT_RA_Level >= 1 ) then
		local sMsg = msg;

		if ( CT_RABoss_Options["ShortRaidAnnounce"] ) then
			sMsg = string.gsub(msg, "%*", "");
		end

		SendChatMessage(sMsg, "RAID_WARNING");

		if ( not CT_RABoss_Options["NoChatSpam"] ) then
