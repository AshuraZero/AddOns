...
--Comparator function for alphabetic sorting of OL maps
local function Atlas_SortZonesAlphaOL(a, b)
	local aa = Atlas_SanitizeName(AtlasOutland[a].ZoneName);
	local bb = Atlas_SanitizeName(AtlasOutland[b].ZoneName);
	return aa < bb;
end
...
function Atlas_UpdateLevelRanges()
	...
	AtlasOutland.HCBloodFurnace.LevelRange =		"61-63";
	AtlasOutland.HCTheShatteredHalls.LevelRange =	"70-72";
	AtlasOutland.HCHellfireRamparts.LevelRange =	"60-62";
	AtlasOutland.HCMagtheridonsLair.LevelRange =	"70+";
	AtlasOutland.CFRTheSlavePens.LevelRange =		"62-64";
	AtlasOutland.CFRTheUnderbog.LevelRange =		"63-65";
	AtlasOutland.CFRTheSteamvault.LevelRange =		"70-72";
...
function Atlas_UpdatePlayerLimits()
	...
	AtlasOutland.HCBloodFurnace.PlayerLimit =		"5";
	AtlasOutland.HCTheShatteredHalls.PlayerLimit =	"5";
	AtlasOutland.HCHellfireRamparts.PlayerLimit =	"5";
	AtlasOutland.HCMagtheridonsLair.PlayerLimit =	"25";
	AtlasOutland.CFRTheSlavePens.PlayerLimit =		"5";
	AtlasOutland.CFRTheUnderbog.PlayerLimit =		"5";
	AtlasOutland.CFRTheSteamvault.PlayerLimit =		"5";
...
function Atlas_Init()
    ...
    for k,v in pairs(AtlasOutland) do table.insert(ATLAS_DROPDOWN_LIST_OL, k); end
	...
	table.sort(ATLAS_DROPDOWN_LIST_OL, Atlas_SortZonesAlphaOL);
...
function AtlasFrameDropDown_Initialize()
	...
	elseif ( AtlasOptions.AtlasType == 3 ) then
		AtlasFrameDropDown_Populate(AtlasOutland, ATLAS_DROPDOWN_LIST_OL);
	elseif ( AtlasOptions.AtlasType == 4 ) then
...
function Atlas_AutoSelect()
	...
	elseif currentType == 3 then
		currentDB = AtlasOutland;
		currentDD = ATLAS_DROPDOWN_LIST_OL;
	else
		return;
	end
	...
	if(currentZone ~= currentMap) then
		...
		for i = 1, getn(ATLAS_DROPDOWN_LIST_OL), 1 do
			local mapName = AtlasOutland[ATLAS_DROPDOWN_LIST_OL[i]]["ZoneName"];
			if(currentZone == mapName) then
