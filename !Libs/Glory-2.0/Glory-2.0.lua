...
elseif locale == "zhCN" then
	PATTERN_HORDE_FLAG_PICKED_UP = "部落的旗帜被([^!]+)拔起了！"
	PATTERN_HORDE_FLAG_DROPPED = "部落的旗帜被(%a+)还到了它的基地中！"
	PATTERN_HORDE_FLAG_CAPTURED = "(%a+)夺取部落的旗帜！"
	PATTERN_ALLIANCE_FLAG_PICKED_UP = "联盟的旗帜被([^!]+)拔起了！"
	PATTERN_ALLIANCE_FLAG_DROPPED = "联盟的旗帜被(%a+)还到了它的基地中！"
	PATTERN_ALLIANCE_FLAG_CAPTURED = "(%a+)夺取联盟的旗帜！"
	
	FACTION_DEFILERS = "污染者"
	FACTION_FROSTWOLF_CLAN = "霜狼氏族"
	FACTION_WARSONG_OUTRIDERS = "战歌氏族"
	
	FACTION_LEAGUE_OF_ARATHOR = "阿拉索联军"
	FACTION_STORMPIKE_GUARD = "雷矛卫队"
	FACTION_SILVERWING_SENTINELS = "银翼要塞"
	...
	BGChatAnnouncements = {
		BGObjectiveClaimedAnnouncements = {
			PATTERN_OBJECTIVE_CLAIMED_AB = "攻占了([%w ]+)！如果不赶快采取行动的话，(%a+)在1分钟内",
		},

		BGObjectiveAttackedAnnouncements = {
			PATTERN_OBJECTIVE_ATTACKED_AB = "突袭了([%w ]+)",
			PATTERN_OBJECTIVE_ATTACKED_AV0 = "夺取了([%w ]+)",
			PATTERN_OBJECTIVE_ATTACKED_AV1 = "^夺取了([%w ]+)",
		},

		BGObjectiveDefendedAnnouncements = {
			PATTERN_OBJECTIVE_DEFENDED_AB = "守住了([%w ]+)",
		},

		BGObjectiveCapturedAnnouncements = {
			PATTERN_OBJECTIVE_CAPTURED_AB = "(%a+)夺取了([%w ]+)",
			PATTERN_OBJECTIVE_CAPTURED_AV0 = "([%w ]+)被(%a+)占领了！",
			PATTERN_OBJECTIVE_CAPTURED_AV1 = "^([%w ]+)被(%a+)占领了！",
			PATTERN_OBJECTIVE_CAPTURED_AV2 = "([%w ]+) is.*MINE", --and the Irondeep Mine is...MINE!
			PATTERN_OBJECTIVE_CAPTURED_AV3 = "claims the ([%w ]+)",   --Snivvle claims the Coldtooth Mine!
		},

		BGObjectiveDestroyedAnnouncements = {
			PATTERN_OBJECTIVE_DESTROYED_AV0 = "([%w ]+)被(%a+)摧毁了！",
			PATTERN_OBJECTIVE_DESTROYED_AV1 = "^([%w ]+)被(%a+)摧毁了！",
		},
	}
	...
	PATTERN_GWSUII_RESOURCES = "资源：(%d+)"   --for lifting the number of bases held in Arathi Basin
	PATTERN_OBJECTIVE_HOLDER = "([%w ]+)控制"
...
local isHorde = (race == RACE_ORC or race == RACE_TROLL or race == RACE_TAUREN or race == RACE_UNDEAD)
