...
function CT_RaidTracker_Delete(id, type, typeid)
	...
	elseif ( type == "player" ) then
		...
		for key, val in CT_RaidTracker_RaidLog[id]["backup"] do
			if ( val["player"] == typeid ) then
				CT_RaidTracker_Debug("DELETE", "JOIN", "FOUND PLAYER", key, val["player"]);
...
function CT_RaidTracker_SortBy(id)
	...
	--DEFAULT_CHAT_FRAME:AddMessage("sort 0 : "..CT_RaidTracker_SortOptions["backup"]);
	CT_RaidTracker_UpdateView();
end
...
function CT_RaidTracker_UpdateView()
	...
	--DEFAULT_CHAT_FRAME:AddMessage("update 0 : "..CT_RaidTracker_SortOptions["backup"]);
	...
	--DEFAULT_CHAT_FRAME:AddMessage("update 1 : "..playerType);			
	--DEFAULT_CHAT_FRAME:AddMessage("update 2 : "..getn(players));		
			for key, val in CT_RaidTracker_RaidLog[raidid][playerType] do
				if ( val["player"] ) then
	...
	--DEFAULT_CHAT_FRAME:AddMessage("update 3 : "..getn(players));	
...
function CT_RaidTracker_RaidsRightClickMenu_Initialize(level)
	...
	if (level == 1) then
		...
		info.func = function()
			HideDropDownMenu(1);
			CT_RaidTrackerGenerateDkpLink(this.value["raidid"],"raid");
		end;
		UIDropDownMenu_AddButton(info, level);
		...
		info.func = function()
			HideDropDownMenu(1);
			CT_RaidTrackerGenerateDkpLink(this.value["raidid"],"backup");
		end;
		UIDropDownMenu_AddButton(info, level);	
		...
		info.func = function()
			HideDropDownMenu(1);
			CT_RaidTrackerGenerateDkpLink(this.value["raidid"],"all");
		end;
		UIDropDownMenu_AddButton(info, level);	
