...
function CT_RA_UpdatePTs()
	local tempOptions = CT_RAMenu_Options["temp"];
	CT_RAPTGroupGroupName:SetText("玩家目标");
	CT_RAPTGroup:Hide();
						...
						if ( name ~= ( frame.unitName or "" ) or hppercent ~= ( frame.hppercent or -1 ) or mppercent ~= ( frame.mppercent or -1 ) or not UnitIsConnected(raidid) ) then
							...
							if ( name == UnitName("player") and not UnitIsPlayer(mtid .. "target") ) then
								...
								if ( not isMT and not CT_RA_UpdateFrame.hasAggroAlert and tempOptions["AggroNotifier"] ) then
									...
									CT_RA_WarningFrame:AddMessage(UnitName(mtid .. "target") .. " 已把目标转向你！", 1, 0, 0, 1, UIERRORS_HOLD_TIME);
				...
				if ( rank >= 2 and CT_RA_Level and CT_RA_Level < 2 ) then
					...
					for j = 1, numRaidMembers, 1 do
						...
						if ( pRank < 1 and pName and CT_RATab_AutoPromotions and CT_RATab_AutoPromotions[pName] ) then
							...
							CT_RA_Print("<团队助手> 自动提升|c00FFFFFF" .. pName .. "|r。", 1, 0.5, 0);
						end
					end
				end
				CT_RA_Level = rank;
...
function CT_RA_MemberFrame_OnEnter()
	...
	if ( not version and not isVirtual ) then
		if ( not stats or not stats["Reporting"] ) then
			GameTooltip:AddLine("没有发现团队助手", 0.7, 0.7, 0.7);
		else
	...
	if ( stats and stats["AFK"] ) then
		if ( type(stats["AFK"][1]) == "string" ) then
			GameTooltip:AddLine("离开：" .. stats["AFK"][1]);
		end
		GameTooltip:AddLine("离开计时：" .. CT_RA_FormatTime(stats["AFK"][2]));
	elseif ( CT_RA_Stats[name] and stats["DND"] ) then
		if ( type(stats["DND"][1]) == "string" ) then
			GameTooltip:AddLine("请勿打扰：" .. stats["DND"][1]);
		end
		GameTooltip:AddLine("请勿打扰计时：" .. CT_RA_FormatTime(stats["DND"][2]));
	end
	if ( stats and stats["Offline"] ) then
		GameTooltip:AddLine("离线计时：" .. CT_RA_FormatTime(stats["Offline"]));
	elseif ( stats and stats["FD"] ) then
		if ( stats["FD"] < 360 ) then
			GameTooltip:AddLine("垂死计时：" .. CT_RA_FormatTime(360-stats["FD"]));
		end
	elseif ( stats and stats["Dead"] ) then
		if ( stats["Dead"] < 360 and not UnitIsGhost(unitid) ) then
			GameTooltip:AddLine("释放计时：" .. CT_RA_FormatTime(360-stats["Dead"]));
		else
			GameTooltip:AddLine("死亡计时：" .. CT_RA_FormatTime(stats["Dead"]));
		end
	end
	if ( stats and stats["Rebirth"] and stats["Rebirth"] > 0 ) then
		GameTooltip:AddLine("复生计时：" .. CT_RA_FormatTime(stats["Rebirth"]));
	elseif ( stats and stats["Reincarnation"] and stats["Reincarnation"] > 0 ) then
		GameTooltip:AddLine("十字章计时：" .. CT_RA_FormatTime(stats["Reincarnation"]));
	elseif ( stats and stats["Soulstone"] and stats["Soulstone"] > 0 ) then
		GameTooltip:AddLine("灵魂石计时：" .. CT_RA_FormatTime(stats["Soulstone"]));
	end
	GameTooltip:Show();
...
function CT_RA_SortByGroup()
	...
	for j = 1, GetNumRaidMembers(), 1 do
		...
		if ( ( not tempOptions["HideOffline"] or online ) and ( CT_RA_Stats[name] or not tempOptions["HideNA"] ) ) then
			...
			frameCache[group].GroupName:SetText("小队 " .. subgroup);
...
function CT_RA_SortByVirtual(updateStatus)
	...
	for j = 1, 40, 1 do
		...
		if ( not name ) then
			isVirtual = 1;
			name, rank, level, class, fileName, zone, online, isDead = "虚拟 " .. i, 0, 60, CT_RA_PRIEST, "PRIEST", "翡翠梦境", 1, nil;
			for i = 1, 8, 1 do
		...
		local groupid = subgroup;
		local group = getglobal("CT_RAGroup" .. groupid);
		frameCache[group].GroupName:SetText("虚拟 " .. subgroup);
		local text = getglobal("CT_RAOptionsGroupButton" .. i .."Texture");
		if ( text ) then
			text:SetVertexColor(1.0, 1.0, 1.0);
		end
	...
	for j = 1, GetNumRaidMembers(), 1 do
		...
		if ( ( not tempOptions["HideOffline"] or online ) and ( CT_RA_Stats[name] or not tempOptions["HideNA"] ) ) then
			...
			if ( name ) then
				...
				local button = getglobal("CT_RAMember" .. i);
				frameCache[group].GroupName:SetText("自定义 " .. subgroup);
				button:ClearAllPoints();
				local new, newName;
				if ( group.next and group.next ~= button ) then
					new = group;
...
function CT_RA_UpdateFrame_OnUpdate(elapsed)
	...
	if ( this.startinviting ) then
		...
		if ( this.startinviting <= 0 ) then
			...
			if ( CT_RA_MinLevel == CT_RA_MaxLevel ) then
				CT_RA_Print("<团队助手> " .. numInvites .. "名等级为|c00FFFFFF" .. CT_RA_MinLevel .. "|r" .. inZone .. "的公会成员已经被邀请。", 1, 0.5, 0);
			else
				CT_RA_Print("<团队助手> " .. numInvites .. "名等级从|c00FFFFFF" .. CT_RA_MinLevel .. "|r到|c00FFFFFF" .. CT_RA_MaxLevel .. "|r" .. inZone .. "的公会成员已经被邀请。", 1, 0.5, 0);
			end
		end
	end
	...
	if ( this.voteTimer ) then
		...
		if ( this.voteTimer <= 0 ) then
			if ( CT_RA_VotePerson ) then
				...
				CT_RA_Print("<团队助手> “|c00FFFFFF" .. CT_RA_VotePerson[4] .. "|r”的投票结果：|c00FFFFFF" .. CT_RA_VotePerson[2] .. "|r (|c00FFFFFF" .. yesPercent .. "%|r) 同意 / |c00FFFFFF" .. CT_RA_VotePerson[3] .. "|r (|c00FFFFFF" .. noPercent .. "%|r) 反对 / |c00FFFFFF" .. noVotes .. "|r (|c00FFFFFF" .. noVotePercent .. "%|r) 没有投票。", 1, 0.5, 0);
				SendChatMessage("<团队助手> “" .. CT_RA_VotePerson[4] .. "”的投票结果：" .. CT_RA_VotePerson[2] .. " (" .. yesPercent .. "%) 同意 / " .. CT_RA_VotePerson[3] .. " (" .. noPercent .. "%) 反对 / " .. noVotes .. " (" .. noVotePercent .. "%) 没有投票。", "RAID");
		...
		if ( this.readyTimer <= 0 ) then
			...
			if ( numNotReady > 0 ) then
				if ( numNotReady == 1 ) then
					CT_RA_Print("<团队助手> " .. notReadyString .. "没有准备好。", 1, 1, 0);
				elseif ( numNotReady >= 8 ) then
					CT_RA_Print("<团队助手> |c00FFFFFF" .. numNotReady .. "|r个团队成员没有准备好。 ", 1, 1, 0);
				else
					CT_RA_Print("<团队助手> |c00FFFFFF" .. numNotReady .. "|r个团队成员（" .. notReadyString .. "）没有准备好。", 1, 1, 0);
				end
				CT_RA_UpdateRaidGroup(1);
			end
			if ( numAfk > 0 ) then
				if ( numAfk == 1 ) then
					CT_RA_Print("<团队助手> " ..afkString .. "处于暂离状态。", 1, 1, 0);
				elseif ( numAfk >= 8 ) then
					CT_RA_Print("<团队助手> |c00FFFFFF" .. numAfk.. "|r个团队成员处于暂离状态。", 1, 1, 0);
				else
					CT_RA_Print("<团队助手> |c00FFFFFF" .. numAfk .. "|r个团队成员（" .. afkString .. "）处于暂离状态。", 1, 1, 0);
				end
				CT_RA_UpdateRaidGroup(1);
			end
		end
	...
	if ( this.rlyTimer ) then
		...
		if ( this.rlyTimer <= 0 ) then
			...
			if ( numNotReady > 0 ) then
				if ( numNotReady == 1 ) then
					CT_RA_Print("<团队助手> " .. notReadyString .. " |c00FFFFFF没准备好|r。", 1, 1, 0);
				elseif ( numNotReady >= 8 ) then
					CT_RA_Print("<团队助手> |c00FFFFFF" .. numNotReady .. "|r 名团队成员没有准备好。", 1, 1, 0);
				else
					CT_RA_Print("<团队助手> |c00FFFFFF" .. numNotReady .. "|r 名团队成员 (" .. notReadyString .. ") 没有准备好。", 1, 1, 0);
				end
				CT_RA_UpdateRaidGroup(1);
			end
			if ( numAfk > 0 ) then
				if ( numAfk == 1 ) then
					CT_RA_Print("<团队助手> " ..afkString .. " 处于暂离状态", 1, 1, 0);
				elseif ( numAfk >= 8 ) then
					CT_RA_Print("<团队助手> |c00FFFFFF" .. numAfk.. "|r 名团队成员处于暂离状态。", 1, 1, 0);
				else
					CT_RA_Print("<团队助手> |c00FFFFFF" .. numAfk .. "|r 名团队成员 (" .. afkString .. ") 处于暂离状态。", 1, 1, 0);
				end
				CT_RA_UpdateRaidGroup(1);
			end
		end
	...
	if ( CT_RA_Squelch > 0 ) then
		...
		if ( CT_RA_Squelch <= 0 ) then
			CT_RA_Squelch = 0;
			CT_RA_Print("<团队助手> 禁言模式已自动停用。（时限已到）", 1, 0.5, 0);
		end
	end
	...
	for k, v in CT_RA_CurrDebuffs do
		...
		if ( CT_RA_CurrDebuffs[k][1] < 0 ) then
			...
			if ( GetBindingKey("CT_CUREDEBUFF") and CT_RA_GetCure(dType) ) then
				msg = "按下'|c00FFFFFF" .. GetBindingText(GetBindingKey("CT_CUREDEBUFF"), "KEY_") .. "|r'进行治疗";
			end
			if ( name == dType ) then
				dType = "";
			else
			...
			if ( CT_RA_CurrDebuffs[k][2] == 1 ) then
				CT_RA_Print("<团队助手> |c00FFFFFF" .. CT_RA_CurrDebuffs[k][4] .. "|r 被施加了 " .. dType .. "：'|c00FFFFFF" .. name .. "|r'" .. msg .. "。", 1, 0.5, 0);
			else
				CT_RA_Print("<团队助手> |c00FFFFFF" .. CT_RA_CurrDebuffs[k][2] .. "|r 个玩家被施加了 " .. dType .. "：'|c00FFFFFF" .. name .. "|r'" .. msg .. "。", 1, 0.5, 0);
			end
...
function CT_RA_UpdateFrame_OnEvent(event)
	...
	elseif ( event == "CHAT_MSG_SYSTEM" ) then
		local _, _, name = string.find(arg1, "^(.+)已经加入了别的队伍$");
		if ( name and this.inviteName and this.inviteName == name ) then
			this.inviteName = nil;
			this.lastInvite = nil;
			SendChatMessage("<团队助手> 你已经有组了。", "WHISPER", nil, name);
		end
	end
end
	...
	for i = 1, numGuildMembers, 1 do
		...
		if ( level >= min and level <= max and name ~= UnitName("player") and not CT_RA_HasInvited[i] and online ) then
			...
			if ( not CT_RA_ZoneInvite or ( CT_RA_ZoneInvite and zone == RealZoneText ) ) then
				table.insert(invList, {i, name});
			end
		end
	end
	local n = table.getn(invList);
	randomseed(GetTime()*1000);
	local needInvites;
	if ( CT_RA_ConvertedRaid ) then
		needInvites = 40 - GetNumRaidMembers();
	else
		needInvites = 40 - GetNumPartyMembers();
	end
	for i = 1, needInvites, 1 do
		if (n == 0) then
			break;
		elseif (n > 1) then
			j = random(1,n);
		...
		InviteByName(invList[j][2]);
		CT_RA_HasInvited[invList[j][1]] = 1;
		numInvites = numInvites + 1;
		table.remove(invList,j);
		n = n - 1;
...
function CT_RA_ScanUnitBuffs(unit, name, id)
	...
	for k, v in oldAuras do
		...
		if ( buffTbl ) then
			if ( not UnitIsDead("raid" .. id) and UnitIsVisible("raid" .. id) and not tempOptions["NotifyDebuffs"]["hidebuffs"] and k ~= CT_RA_POWERWORDSHIELD and k ~= CT_RA_ADMIRALSHAT ) then
				if ( buffTbl["show"] ~= -1 ) then
					if ( CT_RA_CurrPositions[name] ) then
						if ( tempOptions["NotifyDebuffs"][CT_RA_CurrPositions[name][1]] and tempOptions["NotifyDebuffsClass"][CT_RA_ClassPositions[UnitClass("raid" .. CT_RA_CurrPositions[name][2])]] ) then
							if ( CT_RA_ClassSpells and CT_RA_ClassSpells[k] ) then
								if ( GetBindingKey("CT_RECASTRAIDBUFF") ) then
									CT_RA_AddToBuffQueue(k, "raid"..id);
									CT_RA_Print("<团队助手> '|c00FFFFFF" .. name .. "|r' 的 '|c00FFFFFF" .. k .. "|r' 已经消失了。按下 |c00FFFFFF" .. GetBindingText(GetBindingKey("CT_RECASTRAIDBUFF"), "KEY_") .. "|r 来重新施放。", 1, 0.5, 0);
								else
									CT_RA_Print("<团队助手> '|c00FFFFFF" .. name .. "|r' 的 '|c00FFFFFF" .. k .. "|r' 已经消失了。", 1, 0.5, 0);
								end
							end
						end
					end
...
function CT_RA_SetSortType(sort_type)
	...
	if ( sort_type == "class" ) then
		...
		if ( CT_RAMenuFrameGeneralMiscDropDownText ) then
			CT_RAMenuFrameGeneralMiscDropDownText:SetText("职业");
		end
	elseif ( sort_type == "custom" ) then
		...
		if ( CT_RAMenuFrameGeneralMiscDropDownText ) then
			CT_RAMenuFrameGeneralMiscDropDownText:SetText("自定义");
		end
	elseif ( sort_type == "virtual" ) then
		...
		if ( CT_RAMenuFrameGeneralMiscDropDownText ) then
			CT_RAMenuFrameGeneralMiscDropDownText:SetText("虚拟");
		end
	else
		...
		if ( CT_RAMenuFrameGeneralMiscDropDownText ) then
			CT_RAMenuFrameGeneralMiscDropDownText:SetText("小队");
		end
	end
end
...
function CT_RA_CheckGroups()
	...
	if ( tempOptions["NotifyGroupChange"] and ( numjoin > 0 or numleft > 0 ) ) then
		...
		if ( numjoin > 1 ) then
			CT_RA_Print("<团队助手> |c00FFFFFF" .. joined .. "|r已经加入你的队伍。", 1, 0.5, 0);
		elseif ( numjoin == 1 ) then
			CT_RA_Print("<团队助手> |c00FFFFFF" .. joined .. "|r已经加入你的队伍。", 1, 0.5, 0);
		end
		if ( numleft > 1 ) then
			CT_RA_Print("<团队助手> |c00FFFFFF" .. left .. "|r离开了你的队伍。", 1, 0.5, 0);
...
function CT_RA_Emergency_DropDown_Initialize()
	...
	if ( UIDROPDOWNMENU_MENU_VALUE == "职业" ) then
		info = {};
		info.text = "职业";
		info.isTitle = 1;
		info.notCheckable = 1;
		...
		for k, v in CT_RA_ClassPositions do
			if ( ( k ~= CT_RA_SHAMAN or ( UnitFactionGroup("player") and UnitFactionGroup("player") == "Horde" ) ) and ( k ~= CT_RA_PALADIN or ( UnitFactionGroup("player") and UnitFactionGroup("player") == "Alliance" ) ) ) then
				...
				info.tooltipTitle = "切换职业显示";
				info.tooltipText = "切换显示选定职业，可以设定紧急情况监视器中不显示某些职业。";
				UIDropDownMenu_AddButton(info, UIDROPDOWNMENU_MENU_LEVEL);
			end
		end
		return;
	...
	if ( UIDROPDOWNMENU_MENU_VALUE == "小队" ) then
		info = {};
		info.text = "小队";
		info.isTitle = 1;
		info.notCheckable = 1;
		...
		for i = 1, 8, 1 do
			...
			info.keepShownOnClick = 1;
			info.tooltipTitle = "切换小队显示";
			info.tooltipText = "切换显示选定小队，可以设定紧急情况监视器中不显示某些小队。";
			UIDropDownMenu_AddButton(info, UIDROPDOWNMENU_MENU_LEVEL);
		end
		return;
	end
...
function CT_RA_Emergency_DropDown_OnClick()
	...
	if ( UIDROPDOWNMENU_MENU_VALUE == "职业" ) then
		...
		CT_RA_Emergency_UpdateHealth();
	elseif ( UIDROPDOWNMENU_MENU_VALUE == "小队" ) then
...
function InviteElite()
	...
	for i = 1, numGuildMembers, 1 do
		local name, rank, rankIndex, level, class, zone, note, officernote, online, status = GetGuildRosterInfo(i);
		if ( rankIndex == 2 and level == 60 and name ~= UnitName("player") and online) then
			if ( zone == nil ) then zone = "???"; end
			if ( zone == RealZoneText ) then
				InviteByName(name);
			end
		end
	end
