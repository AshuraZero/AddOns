--------------------------------------------------
-- BonusScanner v0.9.6 beta
-- Modified by Archarodim on 2006-04-19 15:20 CEST (performance improve)
...
BonusScanner = {
	...
	MinCheckInterval = 3; -- Minimum time to wait between each scan
	CheckIntervalCounter = 0; -- counter, do not change
	CheckForBonusPlease = 0;
...
function BonusScanner:OnEvent()
    BonusScanner:tit_debug_bis(event);
...
function BonusScanner:tit_debug_bis( Message)
...
function BonusScanner:OnUpdate (elapsed)
    ...
    -- BonusScanner:tit_debug_bis(elapsed);
...
function BonusScanner:ScanEquipment()
	...
	BonusScanner:tit_debug_bis("Scanning Equipment has requested");
